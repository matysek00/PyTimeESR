...
CONTAINS
   ...
   Subroutine Pauli (Nm, N_in, Ndim, Sx_u, Sy_u, Sz_u)
   implicit none
   !F2PY INTENT(IN): Nm 
   !F2PY INTENT(IN): N_in
   !F2PY COMPLEX INTENT(OUT): Sx_u(Nm,Ndim,Ndim)
   !F2PY COMPLEX INTENT(OUT): Sy_u(Nm,Ndim,Ndim)
   !F2PY COMPLEX INTENT(OUT): Sz_u(Nm,Ndim,Ndim)
   integer :: i 
   integer, intent (in) :: Nm, Ndim
   integer, intent (in) :: N_in (:)
   !complex (qc), allocatable, intent(out) :: Sx_u (:,:,:), Sy_u (:,:,:), Sz_u (:,:,:)
   complex (qc), intent(out) :: Sx_u (Nm,Ndim,Ndim), Sy_u (Nm,Ndim,Ndim), Sz_u (Nm,Ndim,Ndim)
